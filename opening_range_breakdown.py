...
import smtplib, ssl
from utils import is_dst, get_latest_weekday
import pandas as pd
import pytz
...
*/1 9-6 * * 1-5 [script-path] >> trade.log 2>&1
...
stocks = cursor.fetchall()  # retrieves all stocks with the opening_range_breakdown strategy applied
...
for symbol in symbols:
    ...
    if not after_opening_range_breakdown.empty:
        if symbol not in existing_order_symbols:
            ...
            messages.append(f"selling short for {symbol} at {limit_price}, closed below {opening_range_low}\n\n{after_opening_range_breakdown.iloc[0]}\n\n")

            print(f"selling short for {symbol} at {limit_price}, closed below {opening_range_low}\n\n{after_opening_range_breakdown.iloc[0]}\n\n")
            ...
            print(f"Opening range breakdown order already made for {symbol}. Skipped")
